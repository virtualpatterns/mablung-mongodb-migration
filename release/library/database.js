...
* Source:       source/library/database.js
...
class Database {
  ...
  async close() {
    if (Is.equal(this.count, 1)) {
      await this.client.close(); // this.database = null
      // this.client = null
    }
  ...
  async existsCollection(name) {
    let cursor = this.database.listCollections({
      'name': name
    });
  ...
  async isMigrationInstalled(name) {
    let migration = await this.database.collection('migration').findOne({
      'name': name,
      'installed': {
        '$gt': 0
      }
  ...
  installMigration(name) {
    return this.database.collection('migration').updateOne({
      'name': name
    }, {
      '$inc': {
        'installed': 1
  ...
  async uninstallMigration(name) {
    ...
    if (Is.not.null(migration)) {
      let history = null;
      history = migration.history.map((history, index) => ({
        index,
        'uninstalled': history.uninstalled
      })).filter(history => Is.null(history.uninstalled)).reverse();
      history = history[0];
...
['createCollection', 'dropCollection', 'collection'].forEach(methodName => {
